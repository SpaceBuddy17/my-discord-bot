...
const COLOR_ROLE_IDS = [
  '1463058233940901892', '1463058244921589770', '1463058237996662950', '1463058235748782256',
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.commandName === 'selfroles') {
    ...
    await interaction.editReply({ content: '✅ Self-roles posted!', ephemeral: true });
  }
});
    ...
    if (MaleFemaleIds.includes(roleId)) {
      ...
      const updatedRows = interaction.message.components.map(row => {
        ...
        newRow.components = newRow.components.map(btn => {
          if (MaleFemaleIds.includes(btn.data.custom_id)) btn.setDisabled(true);
          return btn;
        });
        return newRow;
      });
